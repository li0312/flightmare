'''
Author: Lac_Creeper
Date: 2025-06-16 12:51:59 +0800
LastEditTime: 2025-06-20 22:12:09 +0800
LastEditors: Lac_Creeper
Description: 
FilePath: /flightmare/flightrl/lac_baselines/envs/obstacle_wrapper.py
'''
import numpy as np
...
class ObstEnvVec(VecEnv):
	...
	def __init__(self, impl):
		...
		self._observation_space = spaces.Dict({
			'lidar': spaces.Box(low=-np.inf, high=-np.inf, shape=(3, 512)), 
			'goal': spaces.Box(low=-np.inf, high=np.inf, shape=(2,)), 
	...
	def step(self, action):
		...
		lidar_obs = self._observation[:, :1536]
		goal_obs = self._observation[:, 1536:1538]
		state_obs = self._observation[:, 1538:]
		dict_obs = {"lidar": lidar_obs.reshape([-1, 3, 512]),
								"goal": goal_obs,
								"state": state_obs}

		return dict_obs.copy(), self._reward.copy(), \
	...
	def reset(self):
		...
		for i in range(self.num_envs):
			...
			info[i]['episode'] = epinfo
			self.rewards[i].clear()
		return dict_obs.copy()
	
	def env_is_wrapped(self, wrapper_class, indices=None):
		if indices is None:
			indices = range(self.num_envs)
		return [hasattr(self.wrapper, '_is_wrapped') for _ in indices]
