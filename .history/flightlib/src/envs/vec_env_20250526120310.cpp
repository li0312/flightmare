...
bool VecEnv<EnvBase>::step(Ref<MatrixRowMajor<>> act, Ref<MatrixRowMajor<>> obs,
                           Ref<Vector<>> reward, Ref<BoolVector<>> terminated, 
                           Ref<BoolVector<>> truncated, 
  ...
  if (act.rows() != num_envs_ || act.cols() != act_dim_ ||
      ...
      terminated.rows() != num_envs_ || terminated.cols() != 1 ||
      truncated.rows() != num_envs_ || truncated.cols() != 1 ||
...
#pragma omp parallel for schedule(dynamic)
  for (int i = 0; i < num_envs_; i++) {
    perAgentStep(i, act, obs, reward, terminated, truncated, extra_info);
  }
...
void VecEnv<EnvBase>::testStep(Ref<MatrixRowMajor<>> act,
                               ...
                               Ref<BoolVector<>> terminated,
                               Ref<BoolVector<>> truncated,
                               Ref<MatrixRowMajor<>> extra_info) {
  perAgentStep(0, act, obs, reward, terminated, truncated, extra_info);
...
void VecEnv<EnvBase>::perAgentStep(int agent_id, Ref<MatrixRowMajor<>> act,
  ...
  std::tie(terminated(agent_id), truncated(agent_id)) =
    envs_[agent_id]->isTerminalState(terminal_reward);
  ...
  if (terminated[agent_id] || truncated[agent_id]){
