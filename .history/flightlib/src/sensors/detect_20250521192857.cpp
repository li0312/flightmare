 ...
 * @LastEditTime: 2025-05-21 19:26:02 +0800
...
bool DetectSim::getBBox(const QuadState& state, BBox& out_bbox) {
  ...
  // T_WB_.block<3, 3>(0, 0) = state.R();
  // T_WB_.block<3, 1>(0, 3) = state.p;
  // T_CW_ = T_BC_ * T_BW_;
  ...
  target_cube_.clear();
  target_cube_.emplace_back(5.0f, 0.0f, 1.6f);

  for (const auto& pose : target_cube_) {
    Vector<3> p_body = R_WB_.transpose() * (pose - t_WB_);
    Vector<3> p_camera = R_BC_ * (pose - t_BC_);
    Scalar Z = p_camera.z();
    ...
    logger_.warn("Point: %d, %d", u, v);
    logger_.warn("Point: %.5f", Z);
    std::cout << R_WB_.transpose() << std::endl;
    std::cout << R_BC_ << std::endl;
  }
