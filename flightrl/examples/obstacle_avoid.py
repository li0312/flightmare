'''
Author: Lac_Creeper
Date: 2025-06-24 14:38:04 +0800
LastEditTime: 2025-06-25 15:07:28 +0800
LastEditors: Lac_Creeper
Description: 
FilePath: /flightmare/flightrl/examples/obstacle_avoid.py
'''
...
class TrackPolicy(FeedForwardPolicy):
    ...
    def mlp_extractor(self, flattened_obs, **kwargs):
        ...
        # 1. 拆分展平后的观测
        lidar_data = flattened_obs[:, :lidar_dim]                # shape=(batch, 3*512)
        detect_data = flattened_obs[:, lidar_dim:lidar_dim+detect_dim]  # shape=(batch, 3)
        state_data = flattened_obs[:, -state_dim:]               # shape=(batch, 3)

        # 2. 处理 Lidar 数据（1D CNN）
        lidar_data = tf.reshape(lidar_data, [-1, 3, 512])       # reshape 为 (batch, 3, 512)
...
def main():
    ...
    env = wrapper.FlightEnvVec(ObstEnv_v1(
    ...
    if args.train:
        ...
        model = PPO2(
            ...
            n_steps=300,
            ent_coef=0.00,
            learning_rate=lambda x: lr_schedule.value(x),
            vf_coef=0.5,
            max_grad_norm=0.5,
            nminibatches=1,
            noptepochs=10,
