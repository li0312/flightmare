'''
Author: Lac_Creeper
Date: 2026-03-24 23:15:04 +0800
LastEditTime: 2026-03-25 03:29:10 +0800
LastEditors: Lac_Creeper
Description: 
FilePath: /flightrl/collaborative_detect/demo2/test2.py
'''
import torch
import numpy as np
...
from models2 import LSTMNet, TemporalCNNNet, TransformerNet, CrossAttentionNet
...
def test(args):
    ...
    _, test_loader, _, test_dataset = create_data_loaders(
        data_dir='../dataset2',
        train_meta='../dataset2/test_files.txt', # 实际上这里可以用 test_meta
        val_meta='../dataset2/test_files.txt',
    ...
    with torch.no_grad():
        for i, (batch_x, batch_y) in enumerate(tqdm(test_loader)):
            ...
            for t in range(pred.shape[0]):
                ...
                last_box = batch_x.cpu().numpy()[0, t, 20:24]
            ...
            if i < 10:
                ...
                print("diff_angle: ", calculate_angular_error(pred[-1, 4:], target[-1, 4:]))
...
def visualize_result(pred, target, x, idx, save_dir):
    ...
    for t in range(seq_len):
        ...
        p_box = pred[t, :4]
        t_box = target[t, :4]
        b_box = x[t, 7:11] # 历史帧的 bbox_b 
        l_box = x[t, 20:24] # 历史帧的 bbox_a_last
