'''
Author: Lac_Creeper
Date: 2026-03-24 23:14:40 +0800
LastEditTime: 2026-04-06 20:29:08 +0800
LastEditors: Lac_Creeper
Description: 
FilePath: /flightrl/collaborative_detect/demo2/train2.py
'''
import torch
...
from data_loader2 import create_data_loaders
# 导入模型定义
from models2 import LSTMNet, DirectionNormLoss, TemporalCNNNet, TransformerNet, CrossAttentionNet, CollaborativeTrackingLoss
...
def train_model(args):
    ...
    train_loader, val_loader, train_dataset, _ = create_data_loaders(
        data_dir='./dataset2',
        train_meta='./dataset2/train_files.txt',
        val_meta='./dataset2/val_files.txt',
    ...
    if args.model == 'lstm':
        model = LSTMNet(input_dim=24, hidden_dim=args.hidden_dim).to(device)
    elif args.model == 'attention':
        model = CrossAttentionNet(input_dim=24, hidden_dim=args.hidden_dim).to(device)
    elif args.model == 'transformer':
        model = TransformerNet(input_dim=24, hidden_dim=args.hidden_dim).to(device)


    
    criterion = CollaborativeTrackingLoss(ciou_weight=1.0, dir_weight=5.0, bbox_weight=100.0, dot_weight = 1.0)
    ...
    for epoch in range(args.epochs):
        ...
        for batch_x, batch_y in train_pbar:
            ...
            writer.add_scalar('Loss/Dot', loss_items["loss_dot"], global_step)
            ...
            writer.add_scalars('Train_Step_Comparison', {
                ...
                'Dot_Component': loss_items["loss_dot"]
            }, global_step)
        ...
        # 4. 验证循环
        model.eval()
        val_metrics = {"total": 0, "ciou": 0, "dir": 0, "bbox": 0, "dot": 0}
        total_val_loss = 0
        with torch.no_grad():
            for val_x, val_y in val_loader:
                ...
                val_metrics["dot"] += v_items["loss_dot"]
        ...
        writer.add_scalar('Val_Epoch_Loss/Dot', val_metrics["dot"] / num_val_batches, epoch)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='lstm', choices=['transformer', 'lstm', 'attention'])
