...
class TrackMultEnv final : public EnvBase {
 public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW

  TrackMultEnv();
  TrackMultEnv(const std::string &cfg_path);
  ~TrackMultEnv();
  ...
  friend std::ostream &operator<<(std::ostream &os, const TrackMultEnv &track_env);
 ...
 private:
  ...
  Scalar target_V_;
  Scalar tMaxV_;
  Target target_{Vector<3>{3.0f, 0.0f, 0.0f}, TrajectoryType::TRIANGLE, 1.0f};
  int target_traj_;
  int load_map_{0};
  int map_num_{0};
  ...
  DetectSim detect_;
  Logger logger_{"MultEnv"};

  int step_num_;
  int reach_count_;

  // Define reward for training
  Scalar detect_coeff_, dist_coeff_, alpha_coeff_, beta_coeff_, theta_coeff_, act_coeff_;
  int use_ros_, fine_turn_, random_;
  Scalar traj_param_;
  Scalar randV_;
  int traj_type_;
  Vector<3> targetInitPose_;
  Scalar last_alpha_, last_dist_, last_theta_, last_beta_;
  bool has_init_reward_;

  // observations and actions (for RL)
  Vector<trackMultenv::kNObs> track_obs_;
  Vector<trackMultenv::kNAct> track_act_, last_act_;
  ...
  Vector<trackMultenv::kNAct> act_std_;
