 ...
 * @LastEditTime: 2025-05-27 14:25:30 +0800
...
TrackEnv::TrackEnv(const std::string &cfg_path)
  : EnvBase(),
    ...
    theta_coeff_{0.0f},
    act_coeff_{0.0f} {
  logger_.debug("Test 0..");
  // load configuration file
  if (!std::filesystem::exists(cfg_path)) {
    logger_.debug("Error: Config file not found at ", cfg_path);
  }
  YAML::Node cfg_;
  try {
