...
LastEditTime: 2025-05-25 21:08:03 +0800
...
class TrackEnvVec(VecEnv):
	...
	def __init__(self, impl):
		...
		self._terminated = np.zeros((self.num_envs), dtype=np.bool)
		self._truncated = np.zeros((self.num_envs), dtype=np.bool)
	...
	def step(self, action):
		self.wrapper.step(action, self._observation,
				self._reward, self._terminated, 
				self._truncated, self._extraInfo)
		...
		for i in range(self.num_envs):
			...
			if self._terminated[i] or self._truncated[i]:
				eprew = sum(self.rewards[i])
				eplen = len(self.rewards[i])
				epfinish = self._terminated[i]
				eptimeout = self._truncated[i]
				epinfo = {"r": eprew, "l": eplen, 
						  "f": epfinish, "t": eptimeout}
				info[i]['episode'] = epinfo
				self.rewards[i].clear()

		return self._observation.copy(), self._reward.copy(), \
			self._terminated.copy(), self._truncated.copy(), info.copy()
	
	def stepUnity(self, action, send_id):
		receive_id = self.wrapper.stepUnity(action, self._observation,
                self._reward, self._terminated, 
				self._truncated, self._extraInfo, send_id)
	...
	def reset(self, seed=None, options=None):
		self._reward = np.zeros(self.num_envs, dtype=np.float32)
		self.wrapper.reset(self._observation, seed, options)
