...
LastEditTime: 2025-05-29 15:20:59 +0800
...
class TrackEnvVec(VecEnv):
	...
	def __init__(self, impl):
		...
		self._observation_space = spaces.Dict({
			'lidar': spaces.Box(low=-0.5, high=0.5, shape=(3, 512)), 
			'detect': spaces.Box(low=-0.5, high=0.5, shape=(3,)), 
			'state': spaces.Box(low=-1, high=1, shape=(3,)),
		})
	...
	def step(self, action):
		...
		detect_obs = self._observation[1536:1539]
		state_obs = self._observation[1539:]
		dict_obs = {"lidar": lidar_obs.reshape([3, 512]),
								"detect": detect_obs,
								"state": state_obs}
