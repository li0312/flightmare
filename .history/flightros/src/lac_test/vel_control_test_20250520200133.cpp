 ...
 * @LastEditTime: 2025-05-20 19:59:53 +0800
...
int main(int argc, char *argv[]) {
  ...
  quad_state.x(QS::ATTW) = std::cos(M_PI_4);
  quad_state.x(QS::ATTX) = 0.0;
  quad_state.x(QS::ATTY) = 0.0;
  quad_state.x(QS::ATTZ) = std::sin(M_PI_4);
  ...
  while (ros::ok() && unity_ready) {
    ...
    logger.debug("pose: %.2f, %.2f, %.2f", state4test.p.x(), state4test.p.y(),
                 state4test.p.z());
    logger.debug("body_v: %.2f, %.2f", body_vel_x, body_vel_y);
