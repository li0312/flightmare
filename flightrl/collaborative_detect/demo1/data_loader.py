...
LastEditTime: 2026-03-24 22:24:32 +0800
LastEditors: Lac_Creeper
Description: 
FilePath: /flightrl/collaborative_detect/data_loader.py
'''
import numpy as np
import torch
import json
...
class UAVTrackingDataset(Dataset):
    ...
    def _extract_features(self, data: Dict) -> np.ndarray:
        ...
        # 验证维度
        assert pose_b_a.shape == (4,), f"pose_b_a shape error: {pose_b_a.shape}"
        assert bbox_b.shape == (3,), f"bbox_b shape error: {bbox_b.shape}"
        assert motion_b.shape == (2,), f"motion_b shape error: {motion_b.shape}"
        assert pose_a_delta.shape == (4,), f"pose_a_delta shape error: {pose_a_delta.shape}"
        assert bbox_a_last.shape == (3,), f"bbox_a_last shape error: {bbox_a_last.shape}"
    ...
    def _extract_labels(self, data: Dict) -> np.ndarray:
        ...
        assert bbox_a.shape == (3,), f"bbox_a shape error: {bbox_a.shape}"
    ...
    def _augment_data(self, x: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        x_aug = x.copy()
        x_aug[0:4] += np.random.normal(0, 0.02, 4)      # pose_b_a xyz
        # x_aug[3:7] += np.random.normal(0, 0.02, 4)     # pose_b_a quaternion
        x_aug[9:13] += np.random.normal(0, 0.02, 4)    # pose_a_delta xyz
        # x_aug[16:20] += np.random.normal(0, 0.02, 4)    # pose_a_delta quaternion
        
        # 为bbox添加微小抖动 (±5像素)
        # x_aug[4:7] += np.random.normal(0, 5, 3)        # bbox_b
        # x_aug[13:16] += np.random.normal(0, 5, 3)       # bbox_a_last
        
        # 为运动方向添加噪声
        x_aug[7:9] += np.random.normal(0, 0.05, 2)    # motion_b
        
        # 同样增强输出
        y_aug = y.copy()
        # y_aug[0:3] += np.random.normal(0, 5, 3)         # bbox_a
        y_aug[3:5] += np.random.normal(0, 0.05, 2)      # motion_a
