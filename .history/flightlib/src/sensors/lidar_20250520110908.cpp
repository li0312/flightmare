 ...
 * @LastEditTime: 2025-05-20 11:07:47 +0800
...
Lidar::Lidar()
  ...
  local_vis_.reset(new pcl::PointCloud<pcl::PointXYZ>);
...
void Lidar::lodaClobalPCFromPLY() {
  ...
  kdtreeLocalMap_.setInputCloud(cloud_all_map_);

  if (is_visual_) {
    if (!viewer_->updatePointCloud(cloud_all_map_, "global map")) {
...
bool Lidar::renderPointCloud(const QuadState& state) {
  ...
  if (is_visual_) {
    pcl::visualization::PointCloudColorHandlerGenericField<pcl::PointXYZ>
      local_color(local_vis_, "z");
    if (!viewer_->updatePointCloud(local_vis_,
                                   local_color, "local pc")) {
      viewer_->addPointCloud<pcl::PointXYZ>(local_vis_,
                                            local_color, "local pc");
      viewer_->setPointCloudRenderingProperties(
